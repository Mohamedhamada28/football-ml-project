...
class _LabelEncodedClassifier(BaseEstimator, ClassifierMixin):
    """Thin wrapper that applies LabelEncoder to y before fit/predict.

    Required for XGBClassifier which expects integer class labels,
    not string labels like 'DF', 'FW', 'GK', 'MF'.
    Inherits BaseEstimator + ClassifierMixin for full sklearn compatibility
    including __sklearn_tags__, get_params/set_params, and clone support.
    """

    def __init__(self, estimator: Any) -> None:
        self.estimator = estimator

    def fit(self, X: Any, y: Any, **fit_params: Any) -> "_LabelEncodedClassifier":
        self._le = LabelEncoder()
        y_enc = self._le.fit_transform(y)
        self.estimator_ = clone(self.estimator)
        self.estimator_.fit(X, y_enc, **fit_params)
        self.classes_ = self._le.classes_
        return self

    def predict(self, X: Any) -> Any:
        encoded = self.estimator_.predict(X)
        return self._le.inverse_transform(encoded.astype(int))
...
def evaluate_classification_experiment(
    ...
    for model_name, model in models.items():
        ...
        cv_time_seconds = round(time.perf_counter() - t_cv_start, 2)

        t_fit_start = time.perf_counter()
        model.fit(X_train, y_train)
        fit_time_seconds = round(time.perf_counter() - t_fit_start, 2)

        total_time_seconds = round(cv_time_seconds + fit_time_seconds, 2)
        print(f"  -> CV: {cv_time_seconds}s | Fit: {fit_time_seconds}s | Total: {total_time_seconds}s")
        ...
        for label in labels:
            lm = report.get(label, {})
            sl = label.lower()
            per_class_metrics[f"{sl}_precision"] = float(lm.get("precision", 0.0))
            per_class_metrics[f"{sl}_recall"]    = float(lm.get("recall", 0.0))
            per_class_metrics[f"{sl}_f1"]        = float(lm.get("f1-score", 0.0))

        row: dict[str, Any] = {
            "task": "classification",
            "experiment": experiment,
            "model": model_name,
            "cv_time_s": cv_time_seconds,
            "fit_time_s": fit_time_seconds,
...
def evaluate_regression_experiment(
    ...
    metadata = {"best_model": str(summary.iloc[0]["model"])}
